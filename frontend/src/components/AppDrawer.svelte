<script lang="ts">
    import { Avatar, Drawer, getDrawerStore, getModalStore } from '@skeletonlabs/skeleton';
    ...
    async function deleteChat() {
        if ($drawerStore.meta.owner) {
            drawerStore.close()
            await get('convos/'+$drawerStore.meta.cid+'/delete', {}, $userDataStore.token).then(() => {
                goto(base + "/chatrooms")
            })
        }
    }
...
<Drawer class="align-center text-center mx-auto" on:backdrop={setChat}>
    ...
    {:else if $drawerStore.id == "drawerReplies"}
        <KTextArea sendOnEnter={false} onclick={async (text) => {await $drawerStore.meta.reply(text, $drawerStore.meta.mid)}} ontype={() => {}}/>
