...
<Drawer>
    ...
    <div class="justify-center">
      <div class="rounded-xl bg-gradient-to-br from-primary-500 to-secondary-500 py-4 mb-8 md:mb-2 lg:mb-8">
        {#if photoData}
          <Avatar src={photoData} width="w-20 lg:w-40" rounded="rounded-full" class="mx-auto"/>
        {:else}
          <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto w-20 lg:w-40" enable-background="new 0 0 20 20" viewBox="0 0 20 20" fill="#FFFFFF"><g><rect fill="none" class="w-20 lg:w-40"/></g><g><g><path d="M10 2c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8zm0 3.5c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 11c-2.05 0-3.87-.95-5.07-2.44 1.45-.98 3.19-1.56 5.07-1.56s3.62.58 5.07 1.56c-1.2 1.49-3.02 2.44-5.07 2.44z"/></g></g></svg>
        {/if}
      </div>
      <h2 class="h3 lg:h2 text-center mb-2 md:mb-2 lg:mb-8 max-w-md mx-auto font-medium h-10">{name}</h2>
      <h3 class="h4 lg:h3 text-center mb-8 max-w-md mx-auto h-8">{schoolName}</h3>
      <div class="flex justify-center mb-12 md:m-2 lg:m-12">
