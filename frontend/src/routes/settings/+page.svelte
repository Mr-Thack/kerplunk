...
<div class="grid grid-cols-3 grid-rows-1">
    <div class="col-start-1">
        ...
        <Avatar id="photo-settings" src="https://source.unsplash.com/YOErFW8AfkI/128x128" width="w-40" rounded="rounded-full" class="mx-auto hidden"/>
            <FileButton id="file-button" class="flex justify-center m-4 mx-auto w-fit" button="variant-filled-primary" bind:files={image} on:change={onChangeHandler}></FileButton>
    </div>
    <div class="col-start-2 col-span-2 mx-8">
        <TabGroup>
            <Tab bind:group={tabSet} name="General" value={0} on:click={loadInfo}>General</Tab>
            <Tab bind:group={tabSet} name="Password" value={1}>Password</Tab>
            <Tab bind:group={tabSet} name="Delete" value={2}>Delete</Tab>
            <svelte:fragment slot="panel">
                {#if tabSet === 0}
                    ...
                    <div class="flex justify-center m-12">
                        <button type="button" class="btn variant-filled-primary mx-8 my-4" on:click={loadInfo}>
                            <span>Reset</span>
                        </button>
                ...
                {:else if tabSet === 1}
                <input class="input m-2 w-fill-available moz-available" title="Current Password" type='password' placeholder='Current Password' />
                <input class="input m-2 w-fill-available moz-available" title="New Password" type='password' placeholder='New Password' />
                <input class="input m-2 w-fill-available moz-available" title="Repeat Password" type='password' placeholder='Repeat New Password' />
                <button type="button" class="btn variant-filled-primary mx-8 my-4">
